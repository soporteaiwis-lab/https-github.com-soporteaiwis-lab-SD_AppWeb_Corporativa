...
export const MobileNav = ({ currentRoute, onNavigate }: { currentRoute: AppRoute, onNavigate: (r: AppRoute) => void }) => {
  const items = [
  ...
  return (
    ...
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 lg:hidden z-50 flex justify-around items-center px-2 py-2 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] safe-area-pb">
      {items.map(item => {
        ...
        return (
          <button
            key={item.id}
            onClick={() => onNavigate(item.id)}
            className={`flex flex-col items-center justify-center w-14 py-1 rounded-lg transition-all active:scale-95 ${isActive ? 'text-simple-600' : 'text-slate-400 hover:text-slate-600'}`}
          >
